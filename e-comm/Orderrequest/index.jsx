...
export default function Orderrequest()
{
    ...
    const fetchData = async () => {
      try {
        let t=localStorage.getItem("token");
          ...
          t=t.split(":");
        const response = await fetch("http://localhost:8000/postorderrequest", {
          method: "POST",
  ...
  function loaditem1() {
    const orderedProducts = data.filter((product) => product.state === 'Ordered');
  ...
  async function dispatch(id, email) {
    try {
      let t = localStorage.getItem("token");
      t = t.split(":");
      const response = await fetch("http://localhost:8000/dispatchseller", {
        method: "POST",
        headers: {
  ...
  {
    return (
        data.filter((product) => product.state !== 'Ordered' )
        .map((product)=>(
           <tr key={product.product_id}>
           <td style={{ width: '10%' ,height:'10%'}}>
