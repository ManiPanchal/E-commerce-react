...
export default function Orderrequest()
{
  ...
  function loaditem1() {
    const orderedProducts = data.filter((product) => product.state === 'Dispatched By Seller');
  ...
  async function dispatch(id, email) {
    try {
      let t = localStorage.getItem("token");
      t = t.split(":");
      const response = await fetch("http://localhost:8000/dispatchstate", {
        method: "POST",
        headers: {
  ...
  {
    return (
        data.filter((product) => product.state==="Dispatched By State Dispatcher" || product.state === "Delivered" )
        .map((product)=>(
           <tr key={product.product_id}>
           <td style={{ width: '10%' ,height:'10%'}}>
